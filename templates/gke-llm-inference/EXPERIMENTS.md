...
| 5 | Combine get-credentials and helm upgrade in a single null_resource with triggers | | Ensures credentials are set in fresh CI runners even if resource exists in state; triggers ensure re-deployment on value changes. | Wait for CI |
