...
This template demonstrates how to implement zero-trust egress security using GKE Enterprise FQDN Network Policies. It provisions a private GKE cluster with `ADVANCED_DATAPATH` (Dataplane V2) enabled and registered to a Fleet. The workload is a verifier pod restricted to specific AI API FQDNs (e.g., Anthropic, HuggingFace).

This template provisions:

- **VPC Network** — Dedicated VPC with a primary subnet in `us-central1`
- **GKE Cluster** — Private GKE cluster (`gke-fqdn-egress`) with Spot e2-standard-4 node pool
- **Workload** — FQDN Network Policy and a curl-based verifier pod
...
| Spot Node Pool (1x e2-standard-4) | ~$50 |
| **Total** | **~$125** |
