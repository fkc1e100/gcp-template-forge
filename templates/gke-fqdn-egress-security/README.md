...
This template demonstrates how to implement zero-trust egress security for GKE workloads using **FQDN Network Policies**. It leverages GKE Dataplane V2 and GKE Enterprise (Fleet registration) to allow egress only to specific, approved domains (like Anthropic and HuggingFace) while blocking all other traffic. This is particularly useful for AI workloads that need to interact with external APIs while maintaining a strict security posture.

This template provisions:

- **VPC Network** — Dedicated VPC with a primary subnet in `us-central1`
- **GKE Cluster** — Standard cluster (`gke-fqdn-egress`) with a spot node pool of `e2-standard-4` nodes
- **Workload** — a curl-based egress verifier pod and FQDN network policies
...
Testing domain: google.com (Expected: false)... SUCCESS: google.com is blocked as expected.
