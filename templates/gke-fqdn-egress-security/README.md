...
This template demonstrates how to implement zero-trust egress security in GKE using **FQDN Network Policies**. It provisions a private GKE cluster with **Dataplane V2** enabled, which is required for FQDN-based filtering. The cluster is registered to a **GKE Hub Fleet** to enable GKE Enterprise features, allowing you to restrict outbound traffic from your workloads to specific, approved domains while blocking all other external access.

This template provisions:

- **VPC Network** — Dedicated VPC with a primary subnet and Cloud NAT for private node connectivity.
- **GKE Cluster** — Private Standard cluster (`gke-fqdn-egress`) with Dataplane V2 and FQDN Network Policy enabled.
- **Workload** — A verifier pod that attempts to access allowed (Anthropic, HuggingFace) and blocked FQDNs to confirm policy enforcement.
...
| e2-standard-4 Node Pool (1x nodes) | ~$75 |
| **Total** | **~$150** |

*Estimates based on sustained use in us-central1. GKE Enterprise features may incur additional costs depending on project-level billing.*
...
terraform plan \
  ...
  -var="service_account=YOUR_GKE_SA"
...
terraform destroy -var="project_id=YOUR_PROJECT_ID" -var="region=us-central1" -var="service_account=YOUR_GKE_SA"
```
...
Testing domain: anthropic.com (Expected: true)... SUCCESS: anthropic.com is reachable
...
...
| `service_account` | GKE node service account | required |
