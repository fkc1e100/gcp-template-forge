...
This template demonstrates how to implement zero-trust egress security in GKE using **FQDN Network Policies**. It leverages **GKE Dataplane V2** (powered by eBPF) and **GKE Enterprise** features to restrict outbound traffic from specific Pods to only approved Fully Qualified Domain Names (e.g., `storage.googleapis.com`, `logging.googleapis.com`).

The architecture ensures that even if a Pod is compromised, it cannot reach unauthorized external endpoints, effectively preventing data exfiltration and command-and-control communication to untrusted sites.

This template provisions:

- **VPC Network** — Dedicated VPC with a primary subnet in `us-central1`
- **GKE Cluster** — Regional cluster (`gke-fqdn-egress`) with an e2-standard-4 Spot node pool
- **Workload** — a zero-trust verifier workload that tests egress connectivity to specific FQDNs (e.g., Google APIs)
...
| Spot Node Pool (3x e2-standard-4) | ~$50 |
| Networking & GKE Enterprise | ~$25 |
| **Total** | **~$150** |
...
CLUSTER_NAME=$(kubectl get containerclusters.container.cnrm.cloud.google.com \
  -n forge-management -l "template=gke-fqdn-egress-security" \
...
| `cluster_name` | GKE cluster name | `gke-fqdn-egress-security-cluster` |
| `network_name` | VPC network name | `gke-fqdn-egress-security-vpc` |
| `subnet_name` | Subnet name | `gke-fqdn-egress-security-subnet` |
