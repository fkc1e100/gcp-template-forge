...
This template demonstrates how to implement zero-trust egress security in GKE using **FQDN Network Policies**. It leverages GKE Dataplane V2 to restrict outbound traffic from specific pods to only approved external domains (e.g., Anthropic, Hugging Face), effectively preventing data exfiltration while allowing necessary AI API access.

This template provisions:

- **VPC Network** — Dedicated VPC with a primary subnet in `us-central1`
- **GKE Cluster** — Standard cluster (`gke-fqdn-egress`) with Dataplane V2 and FQDN Network Policy enabled
- **Workload** — An egress verifier pod that attempts to reach various external domains to validate the FQDN Network Policy.

### Resource Naming

| Resource | Terraform + Helm | Config Connector |
|---|---|---|
| GKE Cluster | `gke-fqdn-egress-tf` | `gke-fqdn-egress-kcc-cluster` |
...
| Standard Node Pool (1x e2-standard-4 Spot) | ~$30 |
| GKE Enterprise (Standard tier) | ~$45 |
| **Total** | **~$150** |
