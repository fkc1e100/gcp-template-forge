...
echo "=== Validation: Enterprise GKE Cluster ==="
...
cleanup() {
  rm -f "$KUBECONFIG"
  if [ -n "${JOB_NAME:-}" ] && [ -n "${NAMESPACE_WORKLOAD:-}" ]; then
...
echo "PASS: Workload is Available."

# ── 4. Workload Identity Integration ──────────────────────────────────────────
echo "--- Test 4: Workload Identity Integration ---"
...
if [ -z "$POD_NAME" ]; then
  echo "ERROR: Failed to find workload pod!"
  exit 1
fi
...
if [ -z "$SA_ANNOTATION" ] || [ "$SA_ANNOTATION" = "<WORKLOAD_SA_EMAIL>" ]; then
  ...
  JOB_NAME="test-workload-identity-$(date +%s)"
...
EOF
  ...
  if kubectl wait --for=condition=complete job/"${JOB_NAME}" --timeout=5m -n "${NAMESPACE_WORKLOAD}"; then
    ...
    echo "PASS: Workload Identity validated."
  else
...
# ── 5. Endpoint Interaction ───────────────────────────────────────────────────
echo "--- Test 5: Endpoint Interaction ---"
...
for i in {1..12}; do
  if curl -sf --connect-timeout 5 --max-time 10 "http://${SERVICE_IP}:80/"; then
    echo "PASS: Endpoint test passed!"
    break
  fi
  echo "Endpoint not ready (attempt $i/12)..."
  sleep 30
  if [ "$i" -eq 12 ]; then
    echo "ERROR: Endpoint test failed after 12 attempts!"
    exit 1
  fi
done

echo "=== All Validation Tests PASSED for Enterprise GKE Cluster ==="
