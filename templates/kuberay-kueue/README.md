...
3.  Wait for the cluster to be ready:
    ```bash
    kubectl wait --for=condition=Ready containercluster kuberay-kueue-cluster -n forge-management --timeout=30m
    ```
4.  Configure `kubectl` and deploy the operators and workloads:
    ```bash
    gcloud container clusters get-credentials kuberay-kueue-cluster --region us-central1
