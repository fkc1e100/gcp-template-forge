...
resource "google_container_cluster" "topology_cluster" {
  ...
  resource_labels = {
    ...
    template = var.uid_suffix != "" ? "gke-topology-aware-routing-${var.uid_suffix}" : "gke-topology-aware-routing"
  }
...
resource "google_container_node_pool" "primary_nodes" {
  ...
  # A regional node pool with node_count=1 will have 1 node per zone (3 total).
