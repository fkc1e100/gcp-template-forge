...
if [ -z "${CLUSTER_NAME}" ]; then
  ...
  if gcloud container clusters describe gke-inference-fuse --region ${REGION} --project ${PROJECT_ID} >/dev/null 2>&1; then
    CLUSTER_NAME="gke-inference-fuse"
    echo "Detected Terraform cluster: ${CLUSTER_NAME}"
  elif gcloud container clusters describe gke-inference-fuse-kcc --region ${REGION} --project ${PROJECT_ID} >/dev/null 2>&1; then
    CLUSTER_NAME="gke-inference-fuse-kcc"
    echo "Detected Config Connector cluster: ${CLUSTER_NAME}"
  else
    # Try detecting via list with filter (to handle CI suffixes)
    DETECTED_TF=$(gcloud container clusters list --project ${PROJECT_ID} --filter="name ~ gke-inference-fuse.*-tf" --format="value(name)" --limit 1)
    if [ -n "${DETECTED_TF}" ]; then
      CLUSTER_NAME="${DETECTED_TF}"
      echo "Detected Terraform cluster (suffixed): ${CLUSTER_NAME}"
    else
      DETECTED_KCC=$(gcloud container clusters list --project ${PROJECT_ID} --filter="name ~ gke-inference-fuse.*-kcc" --format="value(name)" --limit 1)
