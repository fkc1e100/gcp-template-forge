...
- **Advanced GCS FUSE Tuning**: Utilizes `fileCacheCapacity`, `fileCacheForRangeRead`, and `metadataCacheTTLSeconds` for optimal performance and persistent caching.
...
2.  **Wait for Readiness**:
    ```bash
    kubectl wait --for=condition=Ready containercluster gke-inference-fuse-kcc -n forge-management --timeout=30m
    ```

3.  **Deploy Workload**:
    *Edit `config-connector-workload/workload.yaml` and replace `<PROJECT_ID>` and `<BUCKET_NAME>` with your actual values.*
    ```bash
    gcloud container clusters get-credentials gke-inference-fuse-kcc --region us-central1
