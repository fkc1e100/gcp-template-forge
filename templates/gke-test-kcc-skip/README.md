...
- **GKE Cluster** — Standard cluster (`gke-test-kcc-skip`) with a minimal node pool
...
|---|---|---|
| GKE Cluster | `gke-test-kcc-skip-<uid>-tf` | `gke-test-kcc-skip-<uid>-kcc` |
| VPC Network | `gke-test-kcc-skip-<uid>-tf-vpc` | `gke-test-kcc-skip-<uid>-kcc-vpc` |
| Subnet | `gke-test-kcc-skip-<uid>-tf-subnet` | `gke-test-kcc-skip-<uid>-kcc-subnet` |
...
```bash
cd templates/gke-test-kcc-skip/terraform-helm

# Initialize with GCS backend (or use local state for testing)
terraform init \
  -backend-config="bucket=YOUR_TF_STATE_BUCKET" \
  -backend-config="prefix=gke-test-kcc-skip/terraform-helm"
...
```bash
cd templates/gke-test-kcc-skip/config-connector
...
CLUSTER_NAME=$(kubectl get containerclusters.container.cnrm.cloud.google.com \
  -n forge-management -l "template=gke-test-kcc-skip" \
...
chmod +x templates/gke-test-kcc-skip/validate.sh
./templates/gke-test-kcc-skip/validate.sh
```
...
| `cluster_name` | GKE cluster name | `gke-test-kcc-skip-tf` |
| `network_name` | VPC network name | `gke-test-kcc-skip-tf-vpc` |
| `subnet_name` | Subnet name | `gke-test-kcc-skip-tf-subnet` |
