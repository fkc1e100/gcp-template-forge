...
- **GKE Cluster** — Standard cluster (`k8s-rbac-mgr`) with a default node pool
- **Workload** — Custom RBAC management configuration
...
| E2 Standard Node Pool (2x e2-standard-4) | ~$134 |
| **Total** | **~$209** |
...
terraform init \
  ...
  -backend-config="prefix=k8s-rbac-manager/terraform-helm"
...
CLUSTER_NAME=$(kubectl get containerclusters.container.cnrm.cloud.google.com \
  -n forge-management -l "template=k8s-rbac-mgr" \
...
| `cluster_name` | GKE cluster name | `k8s-rbac-mgr-tf` |
| `network_name` | VPC network name | `k8s-rbac-mgr-tf-vpc` |
| `subnet_name` | Subnet name | `k8s-rbac-mgr-tf-subnet` |
