...
- **GKE Cluster** — GKE Standard cluster with **Gateway API** enabled (`CHANNEL_STANDARD`) and **Node Pool Auto-provisioning (NAP)**.
- **Node Pool** — `e2-standard-4` nodes (1 node minimum) with **Image Streaming (GCFS)** and **Spot VMs** enabled.
- **Workload** — A demonstration workload utilizing **Native Sidecar Containers**, exposed via **GKE Gateway** (using `Gateway` and `HTTPRoute` resources).
