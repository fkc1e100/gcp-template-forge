...
# Use template.yaml for shortName
SHORT_NAME="$FULL_NAME"
if [ -f "${TEMPLATE_PATH}/template.yaml" ]; then
  parsed_name=$(python3 -c "import yaml, sys; print(yaml.safe_load(open('${TEMPLATE_PATH}/template.yaml')).get('shortName', ''))" 2>/dev/null)
  if [ -n "$parsed_name" ]; then
    SHORT_NAME="$parsed_name"
  fi
elif [ -f "${TEMPLATE_PATH}/.short_name" ]; then
...
# Also replace shortName
if [ "$SHORT_NAME" != "$FULL_NAME" ]; then
  find "${TEMPLATE_PATH}/config-connector" "${TEMPLATE_PATH}/config-connector-workload" -type f -name "*.yaml" -exec sed -i "s/${SHORT_NAME}\(-${UID_SUFFIX}\)\?/${SHORT_NAME}-${UID_SUFFIX}/g" {} + 2>/dev/null || true
fi
...
if [[ "$PROJECT_ID" == "gca-gke-2025" ]]; then
  ...
  if [ "$SHORT_NAME" != "$FULL_NAME" ]; then
    BUCKET_NAME="${SHORT_NAME}-tf-${UID_SUFFIX}-bucket"
  fi
  ...
  if [ -f "$VALUES_PATH" ]; then
    ...
    sed -i "s/bucketName: \"\"/bucketName: \"${BUCKET_NAME}\"/g" "$VALUES_PATH"
  fi

  # Use Python for robust YAML patching of KCC manifests
  python3 -m pip install --quiet pyyaml --break-system-packages || true
  python3 - <<PYEOF
import yaml
import sys
import pathlib
