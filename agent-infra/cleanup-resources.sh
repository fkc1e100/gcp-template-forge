...
if kubectl cluster-info &>/dev/null; then
  ...
  KCC_RESOURCES=$(kubectl get $KCC_TYPES -n $KCC_NAMESPACE -o name | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
...
# Firewalls
FIREWALLS=$(gcloud compute firewall-rules list --project=$PROJECT --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
...
BUCKETS=$(gcloud storage buckets list --project=$PROJECT --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-|gke-inference-tf-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete|gke-gca-2025-forge-tf-state" || true)
...
for RGN in $REGIONS; do
  ROUTERS=$(gcloud compute routers list --project=$PROJECT --regions=$RGN --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
  for R in $ROUTERS; do
...
for i in {1..10}; do
  STILL_THERE=$(gcloud compute routers list --project=$PROJECT --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" | wc -l)
  if [ "$STILL_THERE" -le 0 ]; then
...
# VPN Resources
for RGN in $REGIONS; do
  TUNNELS=$(gcloud compute vpn-tunnels list --project=$PROJECT --regions=$RGN --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
  for T in $TUNNELS; do
    echo "Deleting VPN tunnel: $T in $RGN"
    gcloud compute vpn-tunnels delete $T --region=$RGN --project=$PROJECT --quiet || true
  done
  
  VPNS=$(gcloud compute vpn-gateways list --project=$PROJECT --regions=$RGN --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
  for V in $VPNS; do
...
# L7 Resources
FRULES=$(gcloud compute forwarding-rules list --project=$PROJECT --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
for FR in $FRULES; do
...
# Subnets
for RGN in $REGIONS; do
  SUBNETS=$(gcloud compute networks subnets list --project=$PROJECT --regions=$RGN --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
  for S in $SUBNETS; do
...
# Networks (VPCs)
NETWORKS=$(gcloud compute networks list --project=$PROJECT --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" || true)
...
for i in {1..15}; do
  STILL_THERE=$(gcloud compute networks list --project=$PROJECT --format="value(name)" | grep -E "latest-gke-features-|enterprise-gke-|basic-gke-|gke-kuberay-kueue-multitenant-|gke-inference-fuse-cache-|gke-inf-fuse-cache-|gke-fqdn-egress-security-|gke-topology-aware-routing-" | grep -v -E "repo-agent-standard|krmapihost-kcc-instance|kcc-dash-dont-delete" | wc -l)
  if [ "$STILL_THERE" -le 0 ]; then
