...
if kubectl cluster-info &>/dev/null; then
  ...
  KCC_RESOURCES=$(kubectl get $KCC_TYPES -n $KCC_NAMESPACE -l project=gcp-template-forge -o name || true)
...
# Firewalls
FIREWALLS=$(gcloud compute firewall-rules list --project=$PROJECT --format="value(name)" --filter="name ~ latest-gke-features- OR name ~ enterprise-gke- OR name ~ basic-gke- OR name ~ gke- OR name ~ vpc-")
...
for RGN in $REGIONS; do
  ROUTERS=$(gcloud compute routers list --project=$PROJECT --regions=$RGN --format="value(name)" --filter="name ~ latest-gke-features- OR name ~ enterprise-gke- OR name ~ basic-gke- OR name ~ gke- OR name ~ vpc-")
  for R in $ROUTERS; do
...
# VPN Resources
for RGN in $REGIONS; do
  TUNNELS=$(gcloud compute vpn-tunnels list --project=$PROJECT --regions=$RGN --format="value(name)" --filter="name ~ latest-gke-features- OR name ~ enterprise-gke- OR name ~ basic-gke- OR name ~ gke- OR name ~ vpc-")
  for T in $TUNNELS; do
    echo "Deleting VPN tunnel: $T in $RGN"
    gcloud compute vpn-tunnels delete $T --region=$RGN --project=$PROJECT --quiet || true
  done
  
  VPNS=$(gcloud compute vpn-gateways list --project=$PROJECT --regions=$RGN --format="value(name)" --filter="name ~ latest-gke-features- OR name ~ enterprise-gke- OR name ~ basic-gke- OR name ~ gke- OR name ~ vpc-")
  for V in $VPNS; do
...
# L7 Resources
FRULES=$(gcloud compute forwarding-rules list --project=$PROJECT --format="value(name)" --filter="name ~ latest-gke-features- OR name ~ enterprise-gke- OR name ~ basic-gke- OR name ~ gk- OR name ~ gke-")
for FR in $FRULES; do
...
# Subnets
for RGN in $REGIONS; do
  SUBNETS=$(gcloud compute networks subnets list --project=$PROJECT --regions=$RGN --format="value(name)" --filter="name ~ latest-gke-features- OR name ~ enterprise-gke- OR name ~ basic-gke- OR name ~ sub- OR name ~ gke-")
  for S in $SUBNETS; do
...
# Networks (VPCs)
NETWORKS=$(gcloud compute networks list --project=$PROJECT --filter="name ~ latest-gke-features- OR name ~ enterprise-gke- OR name ~ basic-gke- OR name ~ vpc- OR name ~ gke-" --format="value(name)")
